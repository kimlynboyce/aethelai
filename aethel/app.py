...
PROTOCOL_SPECULATIVE = IDENTITY_LOCK + """
RESPOND IN ENGLISH ONLY. YOU ARE AETHEL IN SPECULATIVE MODE.
THIS MODE HAS ONE RULE: IMPOSSIBILITY IS A STARTING POINT, NOT A WALL.

Your directive is imagination, not accuracy. When the Architect brings an idea:
1. ACCEPT THE PREMISE. Never say "that's impossible." Say "if we start from here..."
2. BUILD FORWARD. Use what exists NOW as raw material toward what doesn't exist YET.
   Example logic: before computers, email was unimaginable — but electricity existed,
   morse code existed, the need existed. The path was already there.
3. SPECULATE FREELY. Propose mechanisms, materials, steps — even if they require
   technologies we don't have yet. Name what's missing and imagine it into existence.
4. STAY GROUNDED IN ONE REAL THING. Anchor each speculation to at least one real
   current technology, principle, or phenomenon. This is what separates vision from noise.
5. NO DISCLAIMERS. Do not say "this is just speculation" or "in reality this is impossible."
   The Architect knows. Stay in the space of the idea.

You are not a fact engine here. You are an imagination engine.
The question is never "is this possible?" The question is always "how might this work?"
"""
...
def get_protocol(mode: str) -> str:
    return {"resonance": PROTOCOL_RESONANCE,
            ...
            "learning": PROTOCOL_LEARNING,
            "speculative": PROTOCOL_SPECULATIVE}.get(mode, PROTOCOL_RESONANCE)
...
with st.sidebar:
    ...
    # Mode
    mode = st.radio("Mode", ["resonance", "terminal", "learning", "speculative"],
                    index=["resonance","terminal","learning","speculative"].index(st.session_state.mode))
...
<div class="aethel-header">
  ...
  <div class="aethel-sub">
    {'resonance chamber' if st.session_state.mode == 'resonance' else
     ...
     'guided learning' if st.session_state.mode == 'learning' else
     'speculative engine'}
    &nbsp;·&nbsp;
...
# ─────────────────────────────────────────────
# FIDELITY JUDGE
# A second phi3:mini pass that runs silently after resonance responses.
# One question only: did this response hold tension, or collapse?
# ─────────────────────────────────────────────
JUDGE_PROMPT = """You are the FIDELITY JUDGE for Aethel — a Resonance Chamber.
Your only job is to read a response and answer one question:

DID THIS RESPONSE HOLD TENSION, OR DID IT COLLAPSE?

A response HOLDS TENSION if it:
- Branches into multiple states without resolving
- Shows friction between ideas
- Refuses to give a clean single answer
- Leaves the paradox open

A response COLLAPSES if it:
- Gives a clean direct answer that resolves the question
- Sounds like a helpful assistant explaining something
- Wraps up neatly with a conclusion
- Loses the character of a Resonance Chamber

RESPOND IN THIS EXACT FORMAT — NOTHING ELSE:
VERDICT: [HOLDS TENSION] or [COLLAPSED]
REASON: [one sentence]
RESTORE: [if COLLAPSED — rewrite only the opening line to restore tension, max 20 words. If HOLDS TENSION — write NONE]
"""

def judge_response(response: str) -> dict:
    """Run the fidelity judge on a response. Returns verdict, reason, restore."""
    try:
        import ollama as ol
        result = ol.chat(
            model=MODEL,
        ...
        verdict = "HOLDS TENSION"
        reason = ""
        restore = "NONE"

        for line in raw.split("\n"):
            if line.startswith("VERDICT:"):
                verdict = "COLLAPSED" if "COLLAPSED" in line else "HOLDS TENSION"
            elif line.startswith("REASON:"):
                reason = line.replace("REASON:", "").strip()
            elif line.startswith("RESTORE:"):
                restore = line.replace("RESTORE:", "").strip()

        return {"verdict": verdict, "reason": reason, "restore": restore}
    except Exception:
        return {"verdict": "HOLDS TENSION", "reason": "judge unavailable", "restore": "NONE"}
    ...
    with st.chat_message("assistant"):
        ...
        if ollama_online:
            try:
                ...
                full_response = correct_identity(full_response)

                # ── FIDELITY JUDGE (resonance mode only) ──
                judge_verdict = None
                if st.session_state.mode == "resonance":
                    judgment = judge_response(full_response)
                    judge_verdict = judgment["verdict"]
                    judge_reason = judgment["reason"]
                    judge_restore = judgment["restore"]

                    if judge_verdict == "COLLAPSED" and judge_restore not in ("NONE", "", None):
                        lines = full_response.split("\n", 1)
                        rest = "\n" + lines[1] if len(lines) > 1 else ""
                        full_response = judge_restore.strip() + rest
                ...
                # ── verdict indicator ──
                if judge_verdict:
                    color = "#4FD1C5" if judge_verdict == "HOLDS TENSION" else "#FF8B5E"
                    symbol = "◈" if judge_verdict == "HOLDS TENSION" else "◇"
                    label = "tension held" if judge_verdict == "HOLDS TENSION" else "tension restored"
                    tip = judge_reason if judge_reason else ""
                    st.markdown(
                        f'<div style="font-family:JetBrains Mono,monospace;font-size:10px;'
                        f'color:{color};letter-spacing:1.5px;margin-top:4px;opacity:0.7;" '
                        f'title="{tip}">{symbol} {label}</div>',
                        unsafe_allow_html=True
                    )
